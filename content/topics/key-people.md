---
title: 主要メンバー
aliases: [主要メンバー]
tags: [dd2030, people]
sources: [raw/minutes/weekly-general-meeting.txt, raw/minutes/project-coreloop.txt, raw/minutes/broad-listening-book-meeting.txt]
created: 2026-04-18
updated: 2026-04-18
---

# 主要メンバー

dd2030プロジェクトに関わる主要な人物の一覧。

## プロジェクト全体

| 人物 | 役割 |
|------|------|
| **安野たかひろ** | 創設者。初期の全体定例主催者。2025年4月に参院選出馬検討を表明し、ボードから離脱方向に。チームみらい代表 |
| **鈴木健（Ken Suzuki）** | dd2030ボードメンバー。[[entities/coreloop|コアループ]]プロジェクトHead。自民党PTにも関与 |

## [[entities/kouchou-ai|広聴AI]] / [[entities/broad-listening-book|ブロードリスニング本]]

| 人物 | 役割 |
|------|------|
| **nishio（西尾泰和）** | [[entities/kouchou-ai|広聴AI]]主要開発者。プラグインシステム設計。台湾章執筆。Audrey Tangとの調整 |
| **Ohki（大木真吾）** | 広聴AIチームリード・渉外。自治体との調整窓口 |
| **tokoroten（中山心太）** | [[entities/broad-listening-book|ブロードリスニング本]]プロジェクトリーダー。1〜3章・技術解説等の主執筆者。farbrain・DivConの開発者 |
| **sumino（角野直子）** | ブロードリスニング本執筆（日テレ章、チームみらい章） |
| **bluemo（青山）** | Cartographer開発者。多元現実代表。AI熟議支援ツール開発 |

## [[entities/coreloop|コアループ]]

| 人物 | 役割 |
|------|------|
| **縄田恵子** | Operation/Process Lead。NRI在籍・元財務省。DP設計・調査会社交渉 |
| **川辺陵真（Ryoma）** | Tech Lead。Code for Japan所属。SODPツール開発 |
| **小倉政貴** | Policy Lead。経産省→ビズリーチ→PoliPoli出身。省庁ヒアリング・専門家調整 |
| **西田尚史** | Chief of Staff / Communication Lead。記者会見・クラファン担当 |
| **赤澤直樹** | Reference Product Lead。ストップ詐欺広告の実装。BeaconLabs所属 |

## [[entities/idobata|いどばた]]

| 人物 | 役割 |
|------|------|
| **kuboon** | いどばた新PO（2025年11月〜）。ドキュメントベース開発推進 |

## [[entities/polimoney|Polimoney]]

| 人物 | 役割 |
|------|------|
| **モアイ（小野翔太）** | コミュニティマネージャー。Polimoney方向性議論。渉外担当 |
| **haruki shimizu** | メンテナー（2025年5月就任） |

## 外部連携

| 人物 | 役割 |
|------|------|
| **オードリー・タン（Audrey Tang）** | 台湾元デジタル相。ブロードリスニング本推薦文。自民党PTでの共同説明 |
| **Alice（Stanford）** | Stanford Deliberative Democracy Lab研究者。SODP開発者。コアループのDP設計パートナー |
| **若尾真也** | Texas大学。日本初のオンラインDP経験者。コアループDP設計アドバイザー |
| **曽根泰教** | 慶應DP研究センター。日本で7回のDP実施経験 |

## 関連ページ

- [[overview]] — プロジェクト全体の概要
- [[topics/community-operations|コミュニティ運営]] — コミュニティの運営体制
